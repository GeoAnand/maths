...
<form id="from1" name="from1" enctype="multipart/form-data"  action="<?php echo $home_path;?>/pages/operation/action/addgallery.php" method="post" class="form-horizontal form-label-left" style="" autocomplete="off">      
...
<td>
 <input type="text" id="intid" name="intid[]" value="<?php if(isset($rw['id']) && $rw['id']!=''){ echo $rw['id'];}else{echo "";} ?>" style="display:none;">   
    <input type="file" id="img<?php echo $x ; ?>" accept=".png, .jpg, .jpeg" name="img[]" class="form-control" value="<?php echo $rw['img'] ; ?>" style="width:100%;"  onkeypress="return pointNum(event)" /><br>
    
    <?php if($rw['img']!='') { ?>
<span id="att1"><?php echo $rw['img'];?></span>&nbsp;
<a href="#"  onclick="selPDFDel('<?php echo $rw['id']; ?>','galpdfview');">Delete</a><br>
<?php } ?>

    </td>
    
<td><input type="text" id="title<?php echo $x ; ?>" name="title[]" class="form-control" value="<?php echo $rw['title'] ; ?>" style="width:100%;"  onkeypress="return pointNum(event)" /></td>
...
<td class='chng' style='width:45px;text-align:center;'>  <a href="<?php echo $home_path; ?>/pages/operation/action/selintgalleryDelete.php?id=<?php echo $rw['id']; ?>" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> Delete </a></td>
</tr>
<?php }  ?>                 
</tbody>
 ...
 function(isConfirm){
   if (isConfirm){
	 ...
	 id=a;
		$.ajax({
	type:'GET',
	url:'<?php echo $home_path; ?>/pages/operation/action/selintgalleryDelete.php',
		data:{
		id:id
		},
		success:function(data){
