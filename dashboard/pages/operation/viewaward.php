...
<form id="from1" name="from1" enctype="multipart/form-data"  action="<?php echo $home_path;?>/pages/operation/action/addaward.php" method="post" class="form-horizontal form-label-left" style="" autocomplete="off">      
...
<?php
$sql="select * from profileaward where fuid='".$_GET['facid']."' order by id ASC";
...
<tr>
    ...
    <td>
 <input type="text" id="gr<?php echo $x ; ?>" name="gr[]" required="required" class="form-control col-md-7 col-xs-12"  style="width:100%;" value="<?php echo $rw['gr']; ?>">


</td>
<td>
 <input type="text" id="yr<?php echo $x ; ?>" name="yr[]" required="required" class="form-control col-md-7 col-xs-12"  style="width:100%;" value="<?php echo $rw['yr']; ?>">

<!--<textarea  id="jauthor<?php echo $x ; ?>" name="yr[]" class="form-control" style="height:125px;width:100%;"><?php if(isset($rw['yr'])) { echo $rw['yr'] ;} ?></textarea>-->
</td>
<td><textarea  id="description<?php echo $x ; ?>" name="description[]" class="form-control" style="height:125px;width:100%;"><?php if(isset($rw['description'])) { echo $rw['description'] ;} ?></textarea></td>
<td class='chng' style='width:45px;text-align:center;'>  <a href="<?php echo $home_path; ?>/pages/operation/action/selawardDelete.php?id=<?php echo $rw['id']; ?>&facid=<?php echo $_GET['facid']; ?>" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> Delete </a></td>
</tr>
<?php }  ?>                 
</tbody>
...
function selCHg(){
	...
	document.location.href="<?php echo $home_path; ?>/pages/operation/viewaward.php?fid=&facid="+facname;
}
...
rowCountR=rowCountR+1; 		
		a++;
		var recRow1='<tr id="rowCount1'+rowCountR+'"><td><input type="text" id="gr'+rowCountR+'" name="gr[]" required="required"  class="form-control" style="width:100%;"></td><td><input type="text" id="yr'+rowCountR+'" name="yr[]" required="required"  class="form-control" style="width:100%;"></td><td><textarea  id="description'+a+'" name="description[]" class="form-control" style="height:125px;width:100%;"></textarea></td><td style="text-align:center;"><a href="javascript:void(0);" onclick="removeRow('+rowCountR+');" name="remove['+rowCountR+']" id="remove_'+ rowCountR +'" class="deleterecord btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> Delete</a></td></tr>';
