...
<form id="from1" name="from1" enctype="multipart/form-data"  action="<?php echo $home_path;?>/pages/operation/action/addresearchs.php" method="post" class="form-horizontal form-label-left" style="" autocomplete="off">    
...
<?php $sql =  "select * from researchs_math order by id asc";
...
<?php
if(isset($_GET['title'])){
 $sql="select * from  rese where title='".$_GET['title']."'";   
}else{
 $sql="select * from  rese";   
}
...
<td>
 <input type="text" id="intid" name="intid[]" value="<?php if(isset($rw['id']) && $rw['id']!=''){ echo $rw['id'];}else{echo "";} ?>" style="display:none;">   
    <input type="text" id="name<?php echo $x ; ?>" name="name[]" class="form-control" value="<?php echo $rw['name'] ; ?>" style="width:100%;"  onkeypress="return pointNum(event)" /></td>
    
<td><input type="text" id="lnk<?php echo $x ; ?>" name="lnk[]" class="form-control" value="<?php echo $rw['lnk'] ; ?>" style="width:100%;"  onkeypress="return pointNum(event)" /></td>

<td class='chng' style='width:45px;text-align:center;'>  <a href="<?php echo $home_path; ?>/pages/operation/action/selintresearchDelete.php?id=<?php echo $rw['id']; ?>" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> Delete </a></td>
</tr>
<?php }  ?>                 
</tbody>
...
function selCHg(){
	...
	document.location.href="<?php echo $home_path; ?>/pages/operation/addresearchs.php?title="+title;
}
 ...
 function(isConfirm){
   if (isConfirm){
	 ...
	 id=a;
		$.ajax({
	type:'GET',
	url:'<?php echo $home_path; ?>/pages/operation/action/selintresearchDelete.php',
		data:{
		id:id
		},
