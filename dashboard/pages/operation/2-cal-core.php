...
class Calendar {
  ...
  function save ($start, $end, $txt, $color, $id=null) {
    ...
    if ($id==null) {
      $sql = "INSERT INTO `calevefix` (`evt_start`, `evt_end`, `evt_text`, `evt_color`) VALUES (?,?,?,?)";
      $data = [$start, $end, $txt, $color];
    } else {
      $sql = "UPDATE `calevefix` SET `evt_start`=?, `evt_end`=?, `evt_text`=?, `evt_color`=? WHERE `evt_id`=?";
  ...
  function del ($id) {
    try {
      $this->stmt = $this->pdo->prepare("DELETE FROM `calevefix` WHERE `evt_id`=?");
  ...
  function get ($month, $year) {
    ...
    // (E2) GET EVENTS
    if(isset($_SESSION['rname']) && $_SESSION['rname']!=''){
    $this->stmt = $this->pdo->prepare("SELECT * FROM `calevefix` WHERE `evt_start` BETWEEN ? AND ? AND `evt_end` BETWEEN ? AND ? AND rname='".$_SESSION['rname']."'");   
    }else{
    $this->stmt = $this->pdo->prepare("SELECT * FROM `calevefix` WHERE `evt_start` BETWEEN ? AND ? AND `evt_end` BETWEEN ? AND ? ");   
    }
    //$this->stmt = $this->pdo->prepare("SELECT * FROM `calevefix` WHERE `evt_start` BETWEEN ? AND ? OR `evt_end` BETWEEN ? AND ?");
...
define('DB_NAME', 'acivil');
define('DB_CHARSET', 'utf8');
define('DB_USER', 'root');
define('DB_PASSWORD', ''); */

define('DB_HOST', 'localhost');
define('DB_NAME', 'matadmin');
define('DB_CHARSET', 'utf8');
define('DB_USER', 'root');
define('DB_PASSWORD', '');
