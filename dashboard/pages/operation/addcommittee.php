...
<form id="from1" name="from1" enctype="multipart/form-data"  action="<?php echo $home_path;?>/pages/operation/action/addcommittee.php" method="post" class="form-horizontal form-label-left" style="" autocomplete="off">    

 <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Title<span class="required" >*</span>
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">				    
  <select name="title" id="title" class="form-control col-md-7 col-xs-12 select2"  onchange="selCHg();">
<option value="">-- Select Division Name --</option>
<?php $sql =  "select * from committee order by id asc";
$val = mysqli_query($conn,$sql);
while($fetch=mysqli_fetch_array($val))
{ 
?>
<?php if($fetch['id']==$_GET['title']){ ?>
<option value="<?=$fetch['id']?>" selected ><?=$fetch['title']?></option>
<?php }else{ ?>
...
<?php
if(isset($_GET['title'])){
 $sql="select * from  committees where title='".$_GET['title']."'";   
}else{
...
<td>
 <input type="text" id="intid" name="intid[]" value="<?php if(isset($rw['id']) && $rw['id']!=''){ echo $rw['id'];}else{echo "";} ?>" style="display:none;">   
    <input type="text" id="topic<?php echo $x ; ?>" name="topic[]" class="form-control" value="<?php echo $rw['topic'] ; ?>" style="width:100%;"  onkeypress="return pointNum(event)" /></td>
    
<td><input type="text" id="name<?php echo $x ; ?>" name="name[]" class="form-control" value="<?php echo $rw['name'] ; ?>" style="width:100%;"  onkeypress="return pointNum(event)" /></td>

<td class='chng' style='width:45px;text-align:center;'>  <a href="<?php echo $home_path; ?>/pages/operation/action/selintcommitteeDelete.php?id=<?php echo $rw['id']; ?>" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> Delete </a></td>
</tr>
<?php }  ?>                 
</tbody>
...
function selCHg(){
	...
	document.location.href="<?php echo $home_path; ?>/pages/operation/addcommittee.php?title="+title;
}
 ...
 function(isConfirm){
   if (isConfirm){
	 ...
	 id=a;
		$.ajax({
	type:'GET',
	url:'<?php echo $home_path; ?>/pages/operation/action/selintcommitteeDelete.php',
		data:{
		id:id
		},
