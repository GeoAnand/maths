...
<form id="from1" name="from1" enctype="multipart/form-data"  action="<?php echo $home_path;?>/pages/operation/action/addcenter.php" method="post" class="form-horizontal form-label-left" style="" autocomplete="off">    
...
<?php $sql =  "select * from center order by id asc";
...
<?php
if(isset($_GET['title'])){
 $sql="select * from  centers where title='".$_GET['title']."'";   
}else{
...
<td class='chng' style='width:45px;text-align:center;'>  <a href="<?php echo $home_path; ?>/pages/operation/action/selintcentersDelete.php?id=<?php echo $rw['id']; ?>" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> Delete </a></td>
</tr>
<?php }  ?>                 
</tbody>
...
function selCHg(){
	...
	document.location.href="<?php echo $home_path; ?>/pages/operation/addcenter.php?title="+title;
}
 ...
 function(isConfirm){
   if (isConfirm){
	 ...
	 id=a;
		$.ajax({
	type:'GET',
	url:'<?php echo $home_path; ?>/pages/operation/action/selintcentersDelete.php',
		data:{
		id:id
		},
