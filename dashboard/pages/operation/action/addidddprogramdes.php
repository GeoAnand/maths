...
$rd=mysqli_fetch_array(mysqli_query($conn,"select MAX(fid)AS fd from programidddnewdes"));
$fd=$rd['fd']+1;

/*echo "select * from programidddnewdes where id='".$_POST['intid']."'";
die();*/
$sq=mysqli_query($conn,"select * from programidddnewdes where id='".$_POST['intid']."'");
...
$sql="insert into programidddnewdes(fuid,fid,deg,des,po,process,sp,syll,cur,ele,note1,note2,note3,status,added_by,added_on)";
$sql.=" values(";
...
$sql.="'".mysqli_real_escape_string($conn,$_POST['des'])."',";
...
$sqqu=mysqli_query($conn,"select * from programidddnewdes where id='".$_POST['intid']."'");
...
$sqlBl="UPDATE programidddnewdes SET "; 
...
$sqlBl=$sqlBl."des='".mysqli_real_escape_string($conn,$_POST['des'])."',";
...
if($UsQuery){
	header('location:'.$home_path.'/pages/operation/viewidddprogramdes.php?msg=success&deg='.$_POST['deg']);
}else{
	header('location:'.$home_path.'/pages/operation/viewidddprogramdes.php?msg=error&deg='.$_POST['deg']);
}
