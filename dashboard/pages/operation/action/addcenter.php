...
if($_POST['lnkname'][$cc]!=''){
$sq=mysqli_query($conn,"select * from centers where id='".$_POST['intid'][$cc]."'");
if(mysqli_num_rows($sq)==0){
$rq=mysqli_fetch_array($sq);

$sql="insert into centers(title,lnkname,lnkl,status,added_by,added_on)";
$sql.=" values(";
...
$sqqu=mysqli_query($conn,"select * from centers where id='".$_POST['intid'][$cc]."'");
$rqqu=mysqli_fetch_array($sqqu);

$sqlBl="UPDATE centers SET "; 
...
if($UsQuery){
	header('location:'.$home_path.'/pages/operation/addcenter.php?msg=success&title='.$_POST['title']);
}else{
	header('location:'.$home_path.'/pages/operation/addcenter.php?msg=error&title='.$_POST['title']);
}
