...
if($_POST['lnkname'][$cc]!=''){
$sq=mysqli_query($conn,"select * from documents where id='".$_POST['intid'][$cc]."'");
if(mysqli_num_rows($sq)==0){
$rq=mysqli_fetch_array($sq);

$sql="insert into documents(lnkname,lnkl,status,added_by,added_on)";
$sql.=" values(";
...
$sqqu=mysqli_query($conn,"select * from documents where id='".$_POST['intid'][$cc]."'");
$rqqu=mysqli_fetch_array($sqqu);

$sqlBl="UPDATE documents SET "; 
...
if($UsQuery){
	header('location:'.$home_path.'/pages/operation/viewdocuments.php?msg=success&facid='.$_POST['fuid']);
}else{
	header('location:'.$home_path.'/pages/operation/viewdocuments.php?msg=error&facid='.$_POST['fuid']);
}
