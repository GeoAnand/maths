...
$rd=mysqli_fetch_array(mysqli_query($conn,"select MAX(fid)AS fd from profilebook"));
$fd=$rd['fd']+1;

for($cc=0;$cc<count($_POST['intareas']);$cc++){
if($_POST['intareas'][$cc]!=''){
$sq=mysqli_query($conn,"select * from profilebook where fuid='".$_POST['fuid']."' AND id='".$_POST['intid'][$cc]."'");
if(mysqli_num_rows($sq)==0){
$rq=mysqli_fetch_array($sq);

$sql="insert into profilebook(fuid,fid,fusrid,intareas,status,added_by,added_on)";
$sql.=" values(";
...
$sqqu=mysqli_query($conn,"select * from profilebook where fid='".$_POST['fid']."' AND fuid='".$_POST['fuid']."'");
$rqqu=mysqli_fetch_array($sqqu);

$sqlBl="UPDATE profilebook SET "; 
...
if($UsQuery){
	header('location:'.$home_path.'/pages/operation/viewbooks.php?msg=success&fid='.$fd.'&facid='.$_POST['fuid']);
}else{
	header('location:'.$home_path.'/pages/operation/viewbooks.php?msg=error&fid='.$fd.'&facid='.$_POST['fuid']);
}
